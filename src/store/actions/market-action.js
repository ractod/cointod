...
const loadButton = document.querySelector(".fa-arrows-rotate")

// اضافه کردن رویداد

navBarSearchIcon.addEventListener("click" , openCloseSearchBar )
...
function searchCoin(event){
    ...
    searchInput.value = ""
    list.innerHTML = ``
    loadButton.classList.toggle("loading")
    fetch(`https://api.coingecko.com/api/v3/search?query=${value}`)
        ...
        .then(json => {
            ...
            fetch(`https://api.coingecko.com/api/v3/coins/${coinId}`)
                ...
                .then(json => {
                    ...
                    li.innerHTML = 
                    `
                    <img src="${json.image.thumb}" width="30px" alt="">
                    <p class="name" >${json.name}</p>
                    <p>${json.symbol}</p>
                    <p>${json.market_data.current_price.usd}$</p>
                    <p>${json.market_cap_rank}</p>
                    <p class="last">${json.market_data.market_cap.usd}$</p>
                    `
                    loadButton.classList.toggle("loading")
                    list.appendChild(li)
                })
        ...
        .catch(() => {
            list.innerHTML = "<h1 class='error'>not founded</h1>"
            loadButton.classList.toggle("loading")
        })
}   
